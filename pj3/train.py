...
def save_checkpoint(state, is_best, task_id, filename='checkpoint.pth.tar', save_dir='/root/ai-fdu.github.io/pj3/model/'):  # 添加保存目录参数
...
class ImgDataset(Dataset):
    def __init__(self, img_dir, tir_dir, gt_dir, shape=None, shuffle=True, transform=None, train=False, batch_size=1, num_workers=4):
    ...
    def __getitem__(self, index):
        ...
        tir_path = os.path.join(
            self.tir_dir, os.path.splitext(img_name)[0]+'R.jpg'
        )
        img, target = load_data(img_path, gt_path, self.train)
        tir, target = load_data(tir_path, gt_path, self.train)
        if self.transform is not None:
            img = self.transform(img)
            tir = self.transform(tir)
...
seed = time.time()
print_freq = 30
img_dir = "/root/ai-fdu.github.io/pj3/dataset/train/rgb/"
gt_dir = "/root/ai-fdu.github.io/pj3/dataset/train/hdf5s/"
tir_dir = "/root/ai-fdu.github.io/pj3/dataset/train/tir/"
pre = None
task = ""
...
def main(args):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train(model, criterion, optimizer, epoch, train_loader, args)
        prec1 = validate(model, val_loader, args)
...
def train(model, criterion, optimizer, epoch, train_loader, args):
    ...
    for i, (img,tir,target) in enumerate(train_loader):
...
def validate(model, val_loader, args):
    ...
    for i, (img, tir, target) in enumerate(val_loader):
        img = img.cuda()
        img = Variable(img)
        tir = tir.cuda()
        tir = Variable(tir)
